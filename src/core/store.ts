...
export type NativeRevenueRunStatus = 'running' | 'success' | 'failed';

export interface NativeRevenueRunRow {
  id: number;
  triggerType: string;
  startDate: string;
  endDate: string;
  mediaFilter: string | null;
  status: NativeRevenueRunStatus;
  phase: string;
  fetchedRows: number;
  insertedRows: number;
  updatedRows: number;
...
let nativeRevenueRunsSchemaReady = false;
const NATIVE_REVENUE_LOCK_NAME = 'ad_tools:native_revenue:global';

async function ensureNativeRevenueRunsSchema(p: mysql.Pool): Promise<void> {
  if (nativeRevenueRunsSchemaReady) return;
  await p.query(
    `CREATE TABLE IF NOT EXISTS native_revenue_runs (
      id BIGINT AUTO_INCREMENT PRIMARY KEY,
      trigger_type VARCHAR(32) NOT NULL,
      start_date DATE NOT NULL,
      end_date DATE NOT NULL,
      media_filter VARCHAR(128) NULL,
      status ENUM('running','success','failed') NOT NULL DEFAULT 'running',
      phase VARCHAR(255) NOT NULL DEFAULT '開始執行',
      ...
      finished_at DATETIME NULL,
      INDEX idx_native_revenue_started (started_at),
      INDEX idx_native_revenue_status (status)
    ) DEFAULT CHARSET=utf8mb4`
  );
  nativeRevenueRunsSchemaReady = true;
}

export async function createNativeRevenueRun(input: {
  triggerType: string; startDate: string; endDate: string; mediaFilter?: string | null;
}): Promise<number> {
  const p = getPool();
  if (!p) throw new Error('DB 未設定');
  await ensureNativeRevenueRunsSchema(p);
  const [res] = await p.query(
    `INSERT INTO native_revenue_runs (trigger_type, start_date, end_date, media_filter)
     VALUES (?, ?, ?, ?)`,
    [input.triggerType, input.startDate, input.endDate, input.mediaFilter ?? null]
  );
  return Number((res as any).insertId);
}

export async function updateNativeRevenueRunPhase(id: number, phase: string): Promise<void> {
  const p = getPool();
  if (!p) return;
  await p.query(`UPDATE native_revenue_runs SET phase=? WHERE id=?`, [phase.slice(0, 255), id]);
}

export async function finishNativeRevenueRun(
  id: number,
  result: { status: 'success' | 'failed'; phase: string; fetchedRows?: number; insertedRows?: number; updatedRows?: number; message?: string }
): Promise<void> {
  const p = getPool();
  if (!p) return;
  await p.query(
    `UPDATE native_revenue_runs SET status=?, phase=?, fetched_rows=?, inserted_rows=?, updated_rows=?,
       message=?, finished_at=NOW() WHERE id=?`,
    [result.status, result.phase.slice(0, 255), result.fetchedRows ?? 0, result.insertedRows ?? 0,
     result.updatedRows ?? 0, result.message ?? null, id]
  );
}

export async function listNativeRevenueRuns(limit = 100): Promise<NativeRevenueRunRow[]> {
  const p = getPool();
  if (!p) return [];
  await ensureNativeRevenueRunsSchema(p);
  const safeLimit = Math.max(1, Math.min(500, Math.floor(limit)));
  const [rows] = await p.query(
    `SELECT id, trigger_type, DATE_FORMAT(start_date, '%Y-%m-%d') AS start_date,
       DATE_FORMAT(end_date, '%Y-%m-%d') AS end_date, media_filter, status, phase,
       fetched_rows, inserted_rows, updated_rows, message,
       DATE_FORMAT(CONVERT_TZ(started_at, '+00:00', '+08:00'), '%Y-%m-%d %H:%i:%s') AS started_at,
       DATE_FORMAT(CONVERT_TZ(finished_at, '+00:00', '+08:00'), '%Y-%m-%d %H:%i:%s') AS finished_at
     FROM native_revenue_runs ORDER BY id DESC LIMIT ${safeLimit}`
  );
  return (rows as any[]).map((r) => ({
    id: Number(r.id), triggerType: r.trigger_type, startDate: r.start_date, endDate: r.end_date,
    mediaFilter: r.media_filter ?? null, status: r.status, phase: r.phase,
    fetchedRows: Number(r.fetched_rows), insertedRows: Number(r.inserted_rows),
    updatedRows: Number(r.updated_rows), message: r.message ?? null,
    startedAt: r.started_at, finishedAt: r.finished_at ?? null,
  }));
}

/** 排程與手動執行共用 advisory lock，避免同時寫入同一份 Sheet。 */
export async function withNativeRevenueRunLock<T>(work: () => Promise<T>): Promise<T> {
  const p = getPool();
  if (!p) throw new Error('DB 未設定');
  const conn = await p.getConnection();
  let locked = false;
  try {
    const [rows] = await conn.query(`SELECT GET_LOCK(?, 0) AS acquired`, [NATIVE_REVENUE_LOCK_NAME]);
    locked = Number((rows as any[])[0]?.acquired ?? 0) === 1;
    if (!locked) throw new Error('已有 Native Revenue 任務執行中，請稍候再試');
    return await work();
  } finally {
    if (locked) await conn.query(`SELECT RELEASE_LOCK(?)`, [NATIVE_REVENUE_LOCK_NAME]);
    conn.release();
  }
}
