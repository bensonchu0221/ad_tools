 ...
 * 直接複用 getAdReportBulk（已含 7 天切段＋分頁），再取 ad_id 去重。
 * ⚠️ bulk 單次區間上限 7 天（>7 天回 code=80008）——切段在 getAdReportBulk 內處理；
 * 任一筆解析失敗會往外拋，由呼叫端退回全打。
 */
export async function getAdReportIndex(
  ...
  const rows = await getAdReportBulk(accessToken, campaignIds, startDate, endDate);
  const dataAdIds = new Set<string>();
  for (const r of rows) if (r?.ad_id != null) dataAdIds.add(String(r.ad_id));
  return dataAdIds;
}
