...
//   * 排程 **`every day 19:00` UTC ＝ 台北 03:00**（2026-09-10 由 UTC 04:00／台北 12:00 提前；
//     Coupang 窗口要求早上 9:00 前收到報表）。schedule 欄位一律是 UTC，看到 19:00 別當成台北 19:00。
...
//   ③ **一定要排在他之後**（排程定在 **台北 04:00**，2026-09-10 由 12:30 改；他台北 03:00 跑、
//      通常 75 秒結束，最壞觀察到延遲 10 分鐘 ⇒ 留 ~1 小時餘裕）。
//      排在他前面寫了就是白寫。一天一次就夠——我們只寫到 T-1，一天內重跑是寫同一份。
//      ⚠️ 台北 04:00 換算 UTC 是前一天 20:00，但 `endDate` 走 `twYmd(now-24h)` 是**台北日**，
//      所以 T-1 仍然正確（不會少算一天）。最近一次 collect 是台北 03:30，T-1 的量已收齊。
