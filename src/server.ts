...
app.get('/', async (_req, reply) => {
  // 卡片套用 daisyUI hover-3d：外層 a 為 hover-3d wrapper，內含 card 與 8 個空 div（滑鼠 hover 偵測區）
  const card = (t: Tool) => `<a class="hover-3d" href="${t.href}"${t.external ? ' target="_blank"' : ''}>
  ...
  const grid = (items: Tool[]) =>
    `<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">${items.map(card).join('')}</div>`;
