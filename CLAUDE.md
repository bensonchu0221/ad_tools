...
- 管線 `src/tools/weeklyreport/report.ts`：並行抓 R（`core/rixbee.ts fetchReport`，metrics 不帶＝回全部含 behavior0-6，7 天一段請求）＋ D（campaign→ad→date_reporting）＋ **M（MGID，`fetchMData` 多帳號序列，見下）** → CV/MCV/MCV2 三桶累加（拖拉分桶，R 用友善名、D 用 cv_* 欄位名、M 用 conv_interest/decision/buy）→ 日/週/素材/受眾聚合
- **MGID（M 平台）併入（2026-07-11，「整合週報」）**：表單加 MGID 帳號**多選** combobox（`listMgidAccounts`，顯示 client_name／值存 api_client_id）＋事件池 3 個靛紫 M chip（`M_EVENTS`＝conv_interest 興趣／conv_decision 決策／conv_buy 購買，固定三階漏斗、非語意事件）。`report.ts fetchMData`：對每個 api_client_id **序列**抓（廣告主 API 併發 6+ 會 429）、`getMgidTokenById` 取 token、`fetchMgidReport`(day×campaign×teaser)＋`fetchMgidDeviceReport`(day×deviceType)→ 標準化 `MRow`；某帳號無 token 只記 warning 跳過、不中斷。**轉換走既有泛用 `calcConversions`**（欄位名比對，MRow 帶 conv_* 欄即通用，零改聚合邏輯）。**teaser 圖**＝`fetchTeaserMetaMap` 多取 `imageLink`（零額外 API，就在既有那支 GET /teasers 回應裡），MGID 素材與 D/R 一起走感知雜湊分群、有縮圖。受眾 key=campaignName（M 無 group）。**MGID 裝置**＝`buildMgidDevice` 每日聚一列寬列、**campaign 欄留空**（day×deviceType 無 campaign 維度，刻意取捨）。驗證：純函式 `poc/verify_weekly_mgid_device.mts`／`verify_weekly_raw_mgid.mts`／`verify_weekly_narrative_mgid.mts`、端到端 `poc/verify_weekly_mgid_e2e.mts`（真 API→xlsx）。設計/計畫見 `docs/superpowers/{specs,plans}/2026-07-11-integrated-weekly-report-mgid*`
...
- Excel `xlsx.ts`（ExcelJS）版型照舊 PHP：7 工作表、素材縮圖 300x157、**Raw 33 欄**（原 30＋尾端 MGID 三階 `conv_interest/decision/buy`；D/R 列這 3 欄補 0、M 列 D/R 專屬事件欄補 0，Raw 無損）；**歷史 quirk：AdAssets 欄放的是 cr_name**（照舊保留）。下載檔名 `weekly_{sd}_{ed}.xlsx`（原 `dr_weekly_`，因已含 M 去前綴）。日/週/素材/受眾/裝置各表零改結構＝M 列透過 report.ts 聚合已併入
