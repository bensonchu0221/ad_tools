...
- 管線 `src/tools/weeklyreport/report.ts`：並行抓 R（`core/rixbee.ts fetchReport`，metrics 不帶＝回全部含 behavior0-6，7 天一段請求）＋ D（campaign→ad→date_reporting）＋ **M（MGID，`fetchMData` 多帳號序列，見下）** → cv1~cv4 四桶累加（拖拉分桶，R 用友善名、D 用 cv_* 欄位名、M 用 conv_interest/decision/buy）→ 日/週/素材/受眾聚合。**2026-07-12 由 CV/MCV/MCV2 三桶語意名改泛用 cv1~cv4 四桶**（對齊 Report Hub）；**隱含 base 照舊映射**：`calcConversions` 桶底 cv1←row.cv、cv2←row.mcv、cv3←row.mcv2、cv4 純拖拉無 base → cv1~cv3 與舊三桶同拖法數字全等（`poc/verify_4buckets_equiv.mts` 對改前 baseline 逐 section 對數通過、cv4 空桶全 0）。`MetricAgg`/`calcConversions`/`addTo` 皆四桶；narrative 只用第一桶 cv1（主要轉換）
