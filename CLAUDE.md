...
## D&R 週報核心（tool#2）
- 管線 `src/tools/weeklyreport/report.ts`：並行抓 R（`core/rixbee.ts fetchReport`，metrics 不帶＝回全部含 behavior0-6）＋ D（campaign→ad→date_reporting，campaign 結束逾 1 個月不抓）→ CV/MCV/MCV2 三桶累加（拖拉分桶，R 用友善名、D 用 cv_* 欄位名）→ 日/週/素材/受眾聚合
...
- 產出走 job 輪詢（TTL 10 分），不同步回傳——抓取可能數十秒
