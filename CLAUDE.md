...
- D 抓取欄位：base 13 欄走 bulk `date, imp, click, ctr, cpc, cpm, charge, cv, cvr, mcv, campaign_id, campaign_name, ad_id`；**cv_* 細分 8 欄**（`cv_view_content, cv_add_to_cart, cv_app_install, cv_complete_registration, cv_add_paymentInfo, cv_start_checkout, cv_search, cv_add_to_wishlist`，與週報 xlsx 口徑一致）bulk 無、另打 **per-ad date_reporting** 取得。寫 sheet 前再補 `account_name, synced_at` → **共 23 欄**
...
- AdStream D 端 2026-06-16 加 cv_* 8 欄（sheet 15→23 欄，走 per-ad）：**線上待驗**——需清空重抓（刪設定重建，游標回到回補起始日）後，確認 cv_* 欄真有值（重點驗 bulk date 與 per-ad date 格式一致、merge 鍵對得上，否則 cv_* 會全 0）
