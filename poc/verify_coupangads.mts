...
check('DAILY_BUDGET 是 4170（2026-09-15 由 2500 調高）', DAILY_BUDGET === 4170, DAILY_BUDGET);
...
check('日預算 4170（2026-09-15 由 2500 調高）', CAMPAIGNS[0].dayBudget === 4170, CAMPAIGNS[0].dayBudget);
check('DAILY_BUDGET 是在跑那幾支的加總（退役的不算進總額）',
  DAILY_BUDGET === CAMPAIGNS.reduce((a, c) => a + c.dayBudget, 0) && DAILY_BUDGET === 4170, DAILY_BUDGET);
check('20 檔 → 每檔 417', budgetPerGroup(CAMPAIGNS[0].dayBudget, 20) === 417, budgetPerGroup(CAMPAIGNS[0].dayBudget, 20));
check('沒有覆蓋時就用設定值', campaignBudget(CAMPAIGNS[0]) === 4170, campaignBudget(CAMPAIGNS[0]));
...
// 使用者每天 14:00 手動關掉最差的一半（20 檔關 10 檔）：剩下那一半的 group 預算加總必須吃得下整個 campaign 日預算
check('20 檔關掉一半後，剩 10 檔的 group 預算加總仍 ≥ 日預算（×2 倍數的用意）',
  10 * budgetPerGroup(CAMPAIGNS[0].dayBudget, 20) >= CAMPAIGNS[0].dayBudget, 10 * budgetPerGroup(CAMPAIGNS[0].dayBudget, 20));
...
check('退役的日預算不進 DAILY_BUDGET（總花費上限只看在跑的）',
  DAILY_BUDGET === 4170 && RETIRED_CAMPAIGNS.reduce((a, c) => a + c.dayBudget, 0) === 0);
{
...
console.log('\n[planRotation：group↔商品永久對映，舊商品回來是「重啟」不是「覆蓋」]');
{
  ...
  check('在跑檔數＝2、每檔 4170（＝日預算 4170÷2×2）', r.activeCount === 2 && r.budgetPerGroup === 4170, r.budgetPerGroup);
}
{
  const ps = [P(1, 'A', 10), P(2, 'B', 20)];
...
console.log('\n[多支 campaign 必須分開算輪替（混在一起會少開一支）——機制保留給下次要再開一支時用]');
{
  ...
  check('每支的每檔預算各算各的（日預算不同 ⇒ 不可以是同一個數字）',
    r1.budgetPerGroup === 4170 && r2b.budgetPerGroup === 1500,
...
console.log('\n[同步摘要]');
{
  const base: any = { campaignIds: [1], campaigns: [{ no: 1 }], retiredPaused: 0, recoCount: 20, unchanged: 20, reimaged: 0, textUpdated: 0, reactivated: 0, created: 0, paused: 0, failed: 0, budgetPerGroup: 300, activeCount: 20, needReview: [], errors: [], elapsedMs: 1234 };
  check('什麼都沒動時不出現雜訊欄位',
    summarize(base) === '不動 20、在跑 20 檔／每檔 300 元、1.2s', summarize(base));
