...
## 寫入（上稿）能力速查 — M 也能建 campaign／素材

完整參數見 `references/advertiser.md`；此處只列「要做跨平台上稿工具時該知道的」：

| 動作 | 端點 |
|---|---|
| 建 campaign | `POST /v1/goodhits/clients/{apiClientId}/campaigns`（必填 `name`≤128唯一、`enabledGeoTargetingFlag`、`geoTargets`、`advertiserName` 1–25字元） |
| 改 campaign | `PATCH .../campaigns/{id}`（多用途：預算/點擊上限、鎖定、IP、widget 篩選；**只傳想改的欄位**） |
| 刪 campaign | `DELETE .../campaigns/{id}`（移垃圾桶） |
| **建素材（teaser）** | `POST /v1/goodhits/clients/{apiClientId}/teasers`（必填 `url`、`campaignId`、`title`≤90、**`imageLink`＝圖片 URL**、`priceOfClick`、`whetherShowGoodPrice`） |
| 改素材／改 CPC／鎖定 | `PUT` ／ `PATCH .../teasers/{id}` |
| 刪素材 | `DELETE .../teasers/{id}` |

跨平台整合時的關鍵差異：
- **M 是兩層**（Campaign→Teaser），**圖片吃 URL**（`imageLink`）——這點跟 D 相同、跟 R 不同（R 要先傳 `/ad-materials` 換 `cr_mt_id`）。
- **`priceOfClick` 以美分計**（D 的 `cpc` 是帳戶幣別，TW 最低 5 TWD）。
- `advertiserName` 帳戶內唯一，撞名回 `[ADVERTISE_NAME_EXISTS]`。
- M **有** DELETE 可回滾；**D 沒有刪除 API**，設計批次流程時 D 要「全部驗證通過才寫入」。

⚠️ **別把 BH 的 M 支援當成上稿**：`r_bulk_upload` repo 的 `services/bh_clients/m_client.py` 只有 `fetch_daily_stats`（打 statistics-reports），是**唯讀報表**。該 repo 目前唯一的上稿寫入路徑是 R（`broadciel_client.py`）。
