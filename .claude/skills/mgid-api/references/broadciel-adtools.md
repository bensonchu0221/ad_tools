...
- **⚠️ statistics-reports 會整個排除「查詢區間內 0 click 的 campaign」——連 imp 都不回**（✅ 已驗 2026-07-13，`poc/verify_mgid_zero_click_campaign.mts`：後台 UI 有 1,043 imp/0 click 的 campaign，API 各維度組合都 0 列；`filters[campaigns][]` 鎖定也一樣）。**單列 click=0 不受影響**：同 campaign 區間內他日有 click，全部日列（含零點擊日）都回、數字與 UI 全等。無參數可關閉此行為。
- **補救端點 `GET /goodhits/clients/{id}/campaigns-stat?dateInterval=interval&startDate=&endDate=`**：campaign 層聚合（無 day/teaser 維度），**不排除零點擊 campaign**，imp 與 UI 全等。回 `{"campaigns-stat":{campaignId:{imps,clicks,spent,avcpc,decision,decisionCost,buying,buyingCost,revenue,epc,profit}}}`——**無 interest 轉換欄、無 adRequests**、金額是純數字（非 {amount,currency}）。ad_tools 的 `fetchMgidReport` 用它做範圍前檢找出缺席（零點擊）campaign。
- **✅ teaser 層救回：`GET /goodhits/clients/{apiClientId}/teaser-stat/{teaserId}?dateInterval=interval&startDate=&endDate=`**（實證 2026-07-22）。回 `{"dateInterval":"…","timezone":"Asia/Taipei","currency":"…","status":"…","teaser-stat":{ "YYYY-MM-DD":{date,clicks,shows,ctr,spent,cpc,interest,decision,buy,interestCost,decisionCost,buyCost} }}`；**無資料回空陣列 `[]`**（要判 `!Array.isArray`）。**關鍵：此端點對「0 點擊 teaser」也回資料**（有 shows）——是唯一能把被 `statistics-reports` 整包排除的零點擊 campaign 曝光**還原到 teaser 層**的來源（同 campaign 各 teaser 的 shows 加總＝campaign imp，實證守恆）。代價：**一次只查一支 teaser（可帶日期區間）**，要救一個零點擊 campaign 得對它**所有 teaser 序列打**（併發 6+ 會 429），數十支起跳。ad_tools `fetchMgidReport`（`fetchZeroClickTeaserRaw`）2026-07-22 起用它把零點擊列補成 teaser 級；救不回（素材為舊 teaser 不在 `/teasers` 清單）才退回 campaigns-stat 的 campaign 級補列並註記。teaser 的 campaignId 直接取自 `/teasers` 清單物件（每筆帶 `campaignId`）。
